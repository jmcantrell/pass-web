...
export default function ({ repository, branch, token = null }) {
  ...
  async function list() {
    const data = await request(`repos/${repository}/git/trees/${branch}?recursive=1`);
    return data.tree.filter((node) => node.type == "blob").map((node) => node.path);
  }

  async function requestPath(path, payload = {}) {
    return await request(`repos/${repository}/contents/${path}`, payload);
  }
  ...
  async function requestPathSHA(path) {
    try {
      ...
      if (e.name == "FetchError" && e.response.status == 404) return null;
      throw e;
    }
  }
  ...
  async function set(path, content, message, update = false) {
    ...
    if (update) body.sha = await requestPathSHA(path);
    await requestPath(path, { method: "PUT", body });
  }

  async function remove(path, message) {
    const body = { message, sha: await requestPathSHA(path) };
    await requestPath(path, { method: "DELETE", body });
  }

  async function updated(path) {
    const commits = await request(`repos/${repository}/commits?path=${path}&per_page=1&page=1`);
